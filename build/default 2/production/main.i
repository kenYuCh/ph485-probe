...
# 14 "/Applications/microchip/xc8/v2.40/pic/include/c90/string.h"
...
States state = STATE_MONITORING_POWER_FOR_SYNC;
...
if( STATE_MONITORING_POWER_FOR_SYNC == state ){
enable_uart_printf();
monitoring_sync(&state, &b1, &b2, &b1_clicked_count, &b2_clicked_count);
_delay((unsigned long)((10)*(1000000/4000.0)));
enable_tx_transmit();
} else if ( STATE_MEASURE == state ){
measurement(&state, &sensor_value);

} else if ( STATE_DISPATCH == state ){
dispatch(&state, &b1, &b2, &b1_clicked_count, &b2_clicked_count);

} else if ( STATE_CALIBRATE == state ){


} else if ( STATE_SAVE_TOP_CALIBRATION_POINT == state ){
printf("STATE_SAVE_TOP_CALIBRATION_POINT\r\n");
...
} else if( STATE_SAVE_BOTTOM_CALIBRATION_POINT == state ){
printf("STATE_SAVE_BOTTOM_CALIBRATION_POINT\r\n");
